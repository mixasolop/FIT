...
def audio_to_midi_basic_pitch(audio_path, midi_output_path):
    ...
    # Clean up any existing MIDI files that might conflict
    base_name = os.path.splitext(os.path.basename(audio_path))[0]
    cleanup_pattern = f"{base_name}*basic_pitch*.mid"
    cleanup_temp_files(cleanup_pattern)
    cleanup_temp_files("*.mid")  # Clean up any .mid files in current directory
    ...
    try:
        ...
        if os.path.exists(expected_midi):
            ...
            # Look for any generated MIDI file
...
def audio_to_midi_pipeline(input_audio, output_midi, stem="other"):
    """
    Complete audio-to-MIDI pipeline
    """
    temp_dir = "temp"
    ...
    # Ensure output_midi has .mid extension
    if not output_midi.endswith('.mid'):
        output_midi = output_midi + '.mid'
    
    audio_to_midi_basic_pitch(separated_path, output_midi)
    return output_midi

def main():
    if len(sys.argv) < 3:
        print("Usage: python audio_to_midi.py <input_audio> <output_midi> [stem]")
        print("stem options: drums, bass, other, vocals (default: other)")
        print("Example: python audio_to_midi.py input.wav output.mid other")
        sys.exit(1)
    ...
    try:
        result_path = audio_to_midi_pipeline(input_audio, output_midi, stem=stem)
        print(f"MIDI conversion complete. Result saved to: {result_path}")
    except Exception as e:
